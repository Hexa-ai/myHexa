...
async function handleSubmit() {
  const ok = await auth.sendMagicLink(email.value)
    ...
    <div class="relative w-full max-w-[440px] fade-up">
      <div class="mb-8 flex flex-col items-center">
        <img
          ...
          draggable="false"
        />
        <h1 class="text-2xl font-semibold tracking-tight">Connexion par email</h1>
        <p class="mt-3 text-sm text-muted-foreground text-center max-w-sm">
          Entrez votre email — nous vous enverrons un lien sécurisé pour vous connecter sans mot de passe.
        </p>
      </div>
      ...
      >
        <p class="text-signal font-mono uppercase tracking-widest text-[11px]">Lien envoyé</p>
        <p>
          Si <span class="font-mono">{{ email }}</span> est connu, un lien de connexion vient
          d'être envoyé. Cliquez sur le lien dans votre boîte (et vérifiez les spams).
        </p>
        <router-link
          to="/login"
      ...
      >
        ...
        >
          <span class="font-mono text-[11px] uppercase tracking-[0.22em]">
            {{ auth.loading ? 'Envoi…' : 'Recevoir le lien' }}
          </span>
        </button>
